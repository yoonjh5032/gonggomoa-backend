...
// - CommonJS export
// - list_url이 검증되지 않은 경우 null + detail_hint.seed_detail_urls 사용
// - include/exclude는 제목 우선, 필요 시 상세 본문에도 적용
// - parser_type은 로컬정부 크롤러에서 분기용으로 사용
...
module.exports = {
  version: '2026-04-03.local-gov-draft.v2',
  ...
  sources: [
    {
      ...
      detail_hint: {
        ...
        seed_detail_urls: [],
        item_link_pattern: /\/site\/gwanak\/ex\/bbsNew\/View\.do\?/i,
        page_param: 'page',
        board_kind: 'bbsNew'
      },
      include_regex:
        /(입찰공고|입찰|용역|제안서|제안서\s*평가위원|평가위원(\s*\(후보자\))?|공개\s*모집|모집\s*공고|수탁기관\s*모집|위탁운영기관\s*모집|사업자\s*모집)/i,
      exclude_regex: COMMON_EXCLUDE_REGEX,
      notes: '관악구는 입찰공고 진입 페이지와 일반 공고 목록을 병행 시드로 사용.'
    },

    {
      ...
      include_regex:
        /(입찰공고|용역|제안서|평가위원(\s*\(후보자\))?|모집\s*공고|위탁운영기관\s*모집|운영기관\s*모집|수탁기관|공개\s*모집|사업자\s*모집)/i,
      exclude_regex:
        /(평가결과|선정결과|개찰결과|낙찰자|우선협상대상자|합격자|채용공고|행정처분|영업정지|등록취소|공시송달|체납자|결산서|후원금|주민등록\s*무단전출)/i,
      notes: '최근 위탁운영기관 모집공고가 실제 확인됨.'
    },

    {
      ...
      exclude_regex: COMMON_EXCLUDE_REGEX,
      notes: '구로구는 상세/첨부 시드 우선. 리스트 URL은 추후 재검증 필요.'
    },

    {
      ...
      notes: '서대문구는 대표 상세 URL 시드 기반. 리스트 주소는 추후 발견 시 교체.'
    },

    {
      ...
      exclude_regex:
        /(평가결과|선정결과|개찰결과|낙찰자|우선협상대상자|결과\s*공고|합격자|채용공고)/i,
      notes: '서초구는 입찰/용역 리스트와 평가위원 모집 상세 시드를 함께 운용.'
    },

    {
      ...
      list_url: 'http://health.yongsan.go.kr/portal/bbs/B0000095/list.do?menuNo=200233',
      detail_hint: {
      ...
      include_regex:
        /(입찰공고|용역|제안서|평가위원(\s*\(후보자\))?|공개\s*모집|모집\s*공고|사업자\s*모집|안전점검\s*수행기관)/i,
      exclude_regex: new RegExp(
        `${COMMON_EXCLUDE_REGEX.source}|${YONGSAN_EXTRA_EXCLUDE_REGEX.source}`,
        'i'
      ),
      notes: '용산구는 채용성 노이즈가 많아 exclude를 강하게 적용.'
    },
...

    {
      ...
      exclude_regex:
        /(평가결과|선정결과|개찰결과|낙찰자|우선협상대상자|결과\s*공고|합격자|채용공고)/i,
      notes: '강서구는 상세 시드 기반으로 시작하고, 현재 운영 중인 리스트 URL 발견 시 교체 권장.'
    }
  ]
};
