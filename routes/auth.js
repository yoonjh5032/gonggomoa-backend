...
router.post('/register', async (req, res) => {
  try {
    ...
    const user  = await User.create({ email, password, nickname, company, phone });
...
router.post('/bookmark/:noticeId', auth, async (req, res) => {
  try {
    const user = await User.findById(req.userId);
    const idx  = user.bookmarks.indexOf(req.params.noticeId);
    if (idx === -1) {
      user.bookmarks.push(req.params.noticeId);
    } else {
