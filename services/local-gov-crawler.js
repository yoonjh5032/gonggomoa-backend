...
function buildListPageUrls(source, maxPages = 3) {
  ...
  for (let page = 1; page <= maxPages; page += 1) {
    try {
      ...
      if (source.parser_type === 'yongsan_health_bbs_list' && !url.searchParams.get('optn1')) {
        url.searchParams.set('optn1', '01');
      }

      urls.push(url.toString());
    } catch (_) {
      if (page === 1) urls.push(listUrl);
    }
  }
...
function extractLinksForSeocho(html, source) {
  ...
  const seedItems = getSeedDetailLinks(source).map((href) => ({ href, text: '' }));
...
function extractLinksForGwanak(html, source) {
  ...
  for (const a of anchors) {
    ...
    if (href && /#view$/i.test(href)) href = '';

    if (!href) {
      const blob = `${a.onclick || ''} ${a.attrsRaw || ''}`;

      const bcIdx = (blob.match(/\bbcIdx['"]?\s*[:=,]\s*['"]?(\d+)['"]?/i) || [])[1];
      const nttId = (blob.match(/\bnttId['"]?\s*[:=,]\s*['"]?(\d+)['"]?/i) || [])[1];
      const bbsIdx = (blob.match(/\bbbsIdx['"]?\s*[:=,]\s*['"]?(\d+)['"]?/i) || [])[1];
      const seq = (blob.match(/\bseq['"]?\s*[:=,]\s*['"]?(\d+)['"]?/i) || [])[1];
      const id = (blob.match(/\bid['"]?\s*[:=,]\s*['"]?(\d+)['"]?/i) || [])[1];

      const candidates = [];
      if (bcIdx) candidates.push(`/site/gwanak/ex/bbsNew/View.do?bcIdx=${bcIdx}`);
      if (nttId) candidates.push(`/site/gwanak/ex/bbsNew/View.do?nttId=${nttId}`);
      if (bbsIdx && nttId) {
        candidates.push(`/site/gwanak/ex/bbs/View.do?bbsIdx=${bbsIdx}&nttId=${nttId}`);
      }
      if (seq) candidates.push(`/site/gwanak/ex/bbsNew/View.do?seq=${seq}`);
      if (id) candidates.push(`/site/gwanak/ex/bbsNew/View.do?id=${id}`);

      href = candidates.map((u) => absoluteUrl(u, baseUrl)).find(Boolean) || '';
    }
...
function buildDateWindow(options = {}) {
  const endDate = normalizeDateInput(options.endDate, { endOfDay: true }) || new Date();
  const startDate =
    normalizeDateInput(options.startDate) ||
    (Number(options.lookbackDays) > 0
      ? new Date(endDate.getTime() - Number(options.lookbackDays) * 24 * 60 * 60 * 1000)
      : null);
...
function extractClosingAt(html) {
  ...
  if (periodMatch) return parseDateTime(periodMatch[3]);

  const deadlineMatch = body.match(
    /(제출기한|마감일시|접수마감|마감)\s*[:：]?\s*(\d{4}[.\-/]\d{1,2}[.\-/]\d{1,2}[^ ]{0,30})/i
  );
  if (deadlineMatch) return parseDateTime(deadlineMatch[2]);
...
async function crawl(options = {}) {
  ...
  const targetSources = onlyKeys ? sources.filter((s) => onlyKeys.has(s.key)) : sources;
  const dateWindow = buildDateWindow(options);
